<?php 
function M_producto_buscado_C($conexion, $id_producto, $id_usuario, $fecha_buscado){//CREA VERIFICANDO DUPLICADOS
	$consulta="SELECT * FROM `mc_productos_buscados` WHERE `ID_PRODUCTO`='$id_producto' AND `ID_USUARIO`='$id_usuario' AND `FH_BUSQUEDA`='$fecha_buscado'";
	$resultado=mysqli_query($conexion,$consulta);
	if(($fila=mysqli_fetch_array($resultado))==true){
		return false;
	}else{
		$fecha_buscado=$fecha_buscado==''?'00-00-00 00:00:00':$fecha_buscado;
		$consulta="INSERT INTO `mc_productos_buscados`(`ID_USUARIO`, `ID_PRODUCTO`, `FH_BUSQUEDA`) VALUES ('$id_usuario', '$id_producto', '$fecha_buscado')";
		$resultados=mysqli_query($conexion,$consulta);
		return true;
	}
}
function M_producto_buscado_R($conexion, $t_1, $f_1, $d_1, $t_2, $f_2, $d_2, $t_3, $f_3, $d_3){
	...
	$consulta="SELECT * FROM `mc_productos_buscados` 
...
function M_producto_buscado_D_id_usuario($conexion, $id){//BORRA DADO EL ID
	$consulta="DELETE FROM `mc_productos_buscados` WHERE `ID_USUARIO`='$id'";
	$resultados=mysqli_query($conexion,$consulta);
	return true;
}
function M_producto_buscado_D_id_producto($conexion, $id){//BORRA DADO EL ID
	$consulta="DELETE FROM `mc_productos_buscados` WHERE `ID_PRODUCTO`='$id'";
	$resultados=mysqli_query($conexion,$consulta);
	return true;
}
function M_producto_buscado_D_id_usuario_y_producto($conexion, $id_usuario, $id_producto){//BORRA DADO EL ID
	$consulta="DELETE FROM `mc_productos_buscados` WHERE `ID_USUARIO`='$id_usuario' AND `ID_PRODUCTO`='$id_producto'";
	$resultados=mysqli_query($conexion,$consulta);
	return true;
}
?>