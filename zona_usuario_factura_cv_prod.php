...
if(isset($_GET['id'])){
	...
	if($datos_previos['ID_TRANSACCION'][0]<>''){
		...
		$pdf->Ln(5);
		$pdf->Cell(190,10,utf8_decode('RECIBO DE COMPRA-VENTA:'), false,0,'C');
		$pdf->Ln();
		...
		$pdf->Cell(25,5,utf8_decode("CV-" . $numero),0,0,'L', true);
		$pdf->Ln();
		$pdf->SetX(140);
		$pdf->Cell(35,5,utf8_decode('Fecha:'),0,0,'R', true);
		//TRATANDO FECHA
		$fecha_imp=explode(" ",$datos_factura['FH_ENTREGADO'][0]);
		...
		$pdf->Cell(70,5,utf8_decode('Descripción'),1,0,'C', true);
		$pdf->Cell(28,5,utf8_decode('Presentación'),1,0,'C', true);
		$pdf->Cell(20,5,utf8_decode('Cantidad'),1,0,'C', true);
		$pdf->Cell(30,5,utf8_decode('Precio Unit (Pm)'),1,0,'C', true);
		$pdf->Cell(35,5,utf8_decode('IMPORTE (Pm)'),1,0,'C', true);
		$pdf->Ln();
		//CUERPO DE LA TABLA
		$pdf->SetFillColor(255,255,255);//Fondo de celda
		$pdf->SetTextColor(0,0,0); //Letra color
		...
		while(isset($datos_factura['ID_TRANSACCION'][$cta])){
			...
			$pdf->Cell(70,5,utf8_decode($datos_factura['NOMBRE_PRODUCTO'][$cta]),1,0,'L', true);
			$dato_del_producto=M_productos_y_servicios_R($conexion, 'CEDULA_RIF', $datos_factura['VENDEDOR_CEDULA_RIF'][$cta], 'NOMBRE_PRODUCTO', $datos_factura['NOMBRE_PRODUCTO'][$cta], '', '');
			$pdf->Cell(28,5,utf8_decode($dato_del_producto['UNIDAD_DE_VENTA'][0]),1,0,'C', true);
			$pdf->Cell(20,5,utf8_decode(number_format($datos_factura['CANTIDAD_COMPRADA'][$cta], 2,',','.')),1,0,'R', true);
			$pdf->Cell(30,5,utf8_decode(number_format($datos_factura['PRECIO_UNITARIO_MICOIN'][$cta], 2,',','.')),1,0,'R', true);
		...
		$pdf->Cell(155,5,utf8_decode("Pemones Pagados por el Comprador:"),0,0,'R', true);
		$pdf->Cell(35,5,utf8_decode(number_format($subtotal, 2,',','.')),1,0,'R', true);
		if($datos_usuario_session['CEDULA_RIF'][0]==$datos_factura['VENDEDOR_CEDULA_RIF'][0]){
			$pdf->Ln();
			...
			$pdf->Ln();
			$pdf->Cell(155,5,utf8_decode("Pemones Comisión Casa Virtual:"),0,0,'R', true);
			$pdf->Cell(35,5,utf8_decode(number_format($comision_cv, 2,',','.')),1,0,'R', true);
			$pdf->Ln();
			$pdf->SetFont('Arial','B',10);
			$pdf->Cell(155,5,utf8_decode("Pemones Recibido por Vendedor:"),0,0,'R', true);
